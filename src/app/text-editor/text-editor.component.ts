...
import { hyphenateSync as hypenateSyncPl } from 'hyphen/pl';
import { hyphenateSync as hypenateSyncEn } from 'hyphen/en';
...
Patterns for this library were taken from ctan.org and ported to Javascript.`;
  ...
  ngOnInit(): void {
    ...
    this.calculateSyllables(this.text.value, hypenateSyncPl);
    ...
    .subscribe(
      (value: string) => {
        ...
        this.calculateSyllables(value, this.hyphenate); 
  ...
  rerenderText(): void {
    if (this.language) {
      this.hyphenate = hypenateSyncEn;
    }
    else {
      this.hyphenate = hypenateSyncPl;
    }
    this.calculateSyllables(this.text.value, this.hyphenate);
  }

  private calculateSyllables(text: string, hyphenateSync: any): void {
